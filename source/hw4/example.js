...
async function getPlaylist(vid, ep) {
    ...
    const key = `${vid}-${ep}`;
    download(vid, ep);
    if (fs.existsSync(path) && downloading[key]?.status !== "listed" && downloading[key]?.status !== "downloading")
...
async function download(vid, ep) {
    ...
    if (!downloading[vid + "-" + ep]) {
        ...
        if (!fs.existsSync(`${dir}/index.m3u8`)) {
            ...
            const cached = await fetch(`https://myself-bbs.jacob.workers.dev/m3u8/${remoteVid}/${remoteEp}?min=1`).then(
                (r) => r.json()
            );
            if (cached.data) {
