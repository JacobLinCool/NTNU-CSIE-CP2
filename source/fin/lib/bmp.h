...
typedef struct BMP {
    ...
    u8 (*save)(struct BMP* bmp, string path, u8 red_bits, u8 green_bits, u8 blue_bits,
               u8 alpha_bits);
    ...
    u64 (*line)(struct BMP* bmp, i64 from_x, i64 from_y, i64 to_x, i64 to_y, u64 width,
                Pixel pixel);
...
static char* BMP_ERROR_MESSAGE[] = {
    "No error", "No File", "Not a BMP", "Invalid BMP header", "Not supported",
};
// #endregion
...
static inline Pixel pixel_over(Pixel front, Pixel back) {
    ...
    if (result.alpha == 0) {
        ...
        result.red =
            (u8)(((f64)front.red * front_alpha + (f64)back.red * back_alpha * (1.0 - front_alpha)) /
                 alpha);
        result.green = (u8)(((f64)front.green * front_alpha +
                             (f64)back.green * back_alpha * (1.0 - front_alpha)) /
                            alpha);
        result.blue = (u8)(((f64)front.blue * front_alpha +
                            (f64)back.blue * back_alpha * (1.0 - front_alpha)) /
                           alpha);
    }
    return result;
}
 ...
 */
u64 bmp_copy(BMP* bmp, i64 from_x, i64 from_y, i64 width, i64 height, BMP* source, i64 source_x,
             i64 source_y) {
    u64 count = 0;
...
u8 write_bmp(BMP* bmp, string path, u8 red_bits, u8 green_bits, u8 blue_bits, u8 alpha_bits) {
    ...
    file_header->reserve_1 = bmp->header->info_header.file_header.reserve_1;
    file_header->reserve_2 = bmp->header->info_header.file_header.reserve_2;
    ...
    for (i32 y = info_header->height - 1; y >= 0; y--) {
        for (i32 x = 0; x < info_header->width; x++) {
            ...
            pixel_data |= (u32)((f64)bmp->pixels[y][x]->green / 0xFF * ((1UL << green_bits) - 1))
                          << blue_bits;
            pixel_data |= (u32)((f64)bmp->pixels[y][x]->red / 0xFF * ((1UL << red_bits) - 1))
                          << (blue_bits + green_bits);
            pixel_data |= (u32)((f64)bmp->pixels[y][x]->alpha / 0xFF * ((1UL << alpha_bits) - 1))
